...
def fill_supplier_reliability_by_group(train_data, test_data, target_column, group_column):
    """Fill missing values in target column using group means from COMBINED data"""
    ...
    print(f"Filled {target_column} using {group_column} group means from COMBINED data")
...
def fill_numerical_with_mean(train_data, test_data, column_name):
    """Replace missing values in numerical column with mean from COMBINED data"""
    ...
    print(f"Filled {column_name} with mean value from COMBINED data: {column_mean:.4f} ")
    return train_filled, test_filled
