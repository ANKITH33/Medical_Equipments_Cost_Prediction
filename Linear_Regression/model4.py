...
def fill_supplier_reliability_by_group(train_data, test_data, target_column, group_column):
    """Fill missing values in target column using group means from group column"""
    print(f"Filling {target_column} using {group_column} group statistics...")
    ...
    # Calculate group means from combined data for consistency
    ...
    print(f"Filled {target_column} using {group_column} group means")
...
def fill_numerical_with_mean(train_data, test_data, column_name):
    """Replace missing values in numerical column with mean"""
    print(f"Filling missing values in {column_name} with mean...")

    train_filled = train_data.copy()
    test_filled = test_data.copy()

    # Calculate mean from combined data for consistency
    ...
    print(f"Filled {column_name} with mean value: {column_mean:.4f}")
    return train_filled, test_filled
...
def create_train_validation_split(train_data, validation_size=0.2, random_seed=42):
...
def build_and_train_ridge_model(train_data, validation_data, test_data, alpha_value=100, model_name="ridge_model"):
    """Build and train Ridge regression model with preprocessing pipeline"""
    print(f"Building Ridge model with alpha={alpha_value}...")
...
def run_complete_modeling_pipeline(train_dataset=None, test_dataset=None, ridge_alpha=0.8, experiment_id="custom_exp"):
    """Execute the complete modeling pipeline"""
    print("="*80)
    print(f"RUNNING COMPLETE MODELING PIPELINE - {experiment_id}")
    print("="*80)
    ...
    train_final, validation_final = create_train_validation_split(train_processed, validation_size=0.01, random_seed=42)
    ...
    # Train model
    trained_model = build_and_train_ridge_model(train_final, validation_final, test_final, ridge_alpha, experiment_id)
...
if __name__ == "__main__":
    model = run_complete_modeling_pipeline()
