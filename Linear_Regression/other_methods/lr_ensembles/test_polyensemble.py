...
def _():
    ...
    return Lasso, LinearRegression, PolynomialFeatures, Ridge, np, pd
...
def _():
    ...
    SUBMISSION_FILE_PATH = 'submission_poly_ensemble.csv'
...
@app.cell
def _():
    # IMPORTANT: UPDATE THESE VALUES after running train_polyensemble.py
    BEST_RIDGE_ALPHA = 73.333 # Replace with the final alpha from your refined Ridge search
    BEST_LASSO_ALPHA = 0.002 # Replace with the final alpha from your refined Lasso search

    return BEST_LASSO_ALPHA, BEST_RIDGE_ALPHA
...
def _(
    ...
    def main():
        ...
        lr_final = LinearRegression().fit(X_train_poly, y_train)
        ridge_final = Ridge(alpha=BEST_RIDGE_ALPHA).fit(X_train_poly, y_train)
        lasso_final = Lasso(alpha=BEST_LASSO_ALPHA, max_iter=30000, tol=0.001).fit(X_train_poly, y_train)
        print("All models trained.")

        # 4. Predict with Each Model and Ensemble
        print("Making predictions with each model...")
        preds_lr = lr_final.predict(X_test_poly)
        preds_ridge = ridge_final.predict(X_test_poly)
        preds_lasso = lasso_final.predict(X_test_poly)

        print("Averaging predictions for final ensemble result...")
        ensemble_preds_log = (preds_lr + preds_ridge + preds_lasso) / 3.0

        # 5. Format and Save Submission
        actual_predictions = np.expm1(ensemble_preds_log)
