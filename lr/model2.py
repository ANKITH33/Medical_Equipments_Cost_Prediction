...
def train_ridge_lasso_with_polynomial():
    """
    Train Ridge and Lasso models with polynomial features and validation set
    """
    ...
    # Define models with adjusted alphas for polynomial features
    models = {
        'Ridge': Ridge(alpha=1000.0, random_state=2, max_iter=5000),  # Higher alpha for regularization
        'Lasso': Lasso(alpha=10.0, random_state=2, max_iter=5000)  # Higher alpha and more iterations
    }
    ...
    for name, model in models.items():
        ...
        if name == 'Lasso':
            ...
            print(f"  Features selected by Lasso: {n_features_selected}/{len(model.coef_)}")
    ...
    if best_model_name == 'Ridge':
        ...
        for i, idx in enumerate(top_indices, 1):
            print(f"{i:2d}. {feature_names[idx]}: {best_model.coef_[idx]:.4f}")

    return results, trained_models, best_model_name, submission, poly, scaler

# Run the training
if __name__ == "__main__":
    print("Starting Ridge and Lasso model training with polynomial features...")
    print("="*80)

    results, models, best_model, submission, poly_transformer, scaler = train_ridge_lasso_with_polynomial()

    print("\n" + "="*80)
    print("MODEL TRAINING WITH POLYNOMIAL FEATURES COMPLETE!")
    print("="*80)
    ...
    print(f"Submission file: submission_polynomial.csv ({submission.shape[0]} predictions)")

    print(f"\nFiles created:")
    print(f"- submission_polynomial.csv")
