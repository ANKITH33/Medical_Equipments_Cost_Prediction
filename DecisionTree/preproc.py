# preproc.py (Updated to prevent data leakage)
...
class Preprocessor:
    ...
    def fit(self, df_train):
        ...
        # --- Learn columns for one-hot encoding ---
    ...
    def transform(self, df_input):
        ...
        # --- 1. Process Target Variable (if it exists) ---
        ...
        # --- 4. Impute All Missing Values using stored values ---
        for col, value in self.imputation_values.items():
            if col in df.columns:
                df[col] = df[col].fillna(value)

        # --- 5. Map Binary Categorical Features ---
        ...
        # --- 6. One-Hot Encode Multi-Category Features ---
        df = pd.get_dummies(df, columns=self.one_hot_columns, drop_first=True, dtype=float)

        # Final check for any remaining NaNs and align columns
